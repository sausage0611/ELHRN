...
def meta_autoparm_train_dataloader(data_path,way1,shot1,way2,shot2,transform_type):

    dataset = get_dataset(data_path=data_path,is_training=True,transform_type=transform_type,pre=None)

    loader = torch.utils.data.DataLoader(
        dataset,
        batch_sampler = samplers.meta_auto_batchsampler(data_source=dataset,way1=way1,shot1=shot1,way2=way2,shot2=shot2),
        num_workers = 3,
        pin_memory = False)
