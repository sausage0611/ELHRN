...
sys.path.append('../../../../')
from trainers import trainer_first, elhrn_train
from datasets import dataloaders
from models.ELHRN import ELHRN


args = trainer_first.train_parser()
with open('../../../../config.yml', 'r') as f:
    temp = yaml.safe_load(f)
data_path = os.path.abspath(temp['data_path'])
fewshot_path = os.path.join(data_path,'CUB_fewshot_cropped')

pm = trainer_first.Path_Manager(fewshot_path=fewshot_path,args=args)
...
model = ELHRN(way=train_way,
            shots=[args.train_shot, args.train_query_shot],
            resnet=args.resnet)

train_func = partial(elhrn_train.ti_train,train_loader=train_loader)

tm = trainer_first.Train_Manager(args,path_manager=pm,train_func=train_func)
